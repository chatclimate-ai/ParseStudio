...
class Metadata(BaseModel):
    ...
    page_number: int = Field(
        None, description="The page number where the element is located."
    )
    bbox: list[float] = Field(
        None, description="Bounding box coordinates of the element."
    )
    ...
    def validate_bbox(cls, bbox):
        if (
            ...
            or len(bbox) != 4
            or not all(isinstance(i, float) for i in bbox)
        ):
...
class TableElement(BaseModel):
    ...
    dataframe: pd.DataFrame = Field(
        None, description="The pandas DataFrame representation of the table."
    )
